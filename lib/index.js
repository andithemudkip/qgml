...
let parser = require ('htmljs-parser').createParser ({
    ...
    onOpenTag: async function (event) {
        ...
        switch (event.tagName) {
            ...
            case 'sprite':
                let spr = new Sprite ();
                spr.setConfigFromAttribs (event.attributes);
                preloadedSprites.push (spr);
                break;
            case 'script':
                if (!event.selfClosed) {
    ...
    onCloseTag: function (event) {
        ...
        switch (event.tagName) {
            ...
            case 'script':
                ...
                break;
            case 'text':
                currentText.group = groupNames [groupDepth];
...
let compile = async (input, output, include = [], standalone = false) => {
    ...
    if (fs.existsSync (inputPath)) {
        ...
        try {
            ...
            for (let i = 0; i < preloadedSprites.length; i++) {
                await preloadedSprites [i].processAssets (assets, inputPath);
            }
