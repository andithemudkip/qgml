...
let compile = (input, output, include = [], standalone = false) => {
    let outstr = "";
    originalInput = input;
    originalOutput = output;
    originalInclude = include;
    if (!standalone) include = ['p5.min.js', 'gml-core.min.js' , ...include]
    ...
    if (fs.existsSync (inputPath)) {
        ...
        try {
            ...
            outstr += `let gml_config = ${JSON.stringify (config)};GML.setup (gml_config);`;
